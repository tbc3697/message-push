...
public class MyRequestHandler extends SimpleChannelInboundHandler {
    ...
    private class HeartbeatTask implements Runnable {
        ...
        @Override
        public void run() {
            MyPushMessage heartbeatPushMessage = MyPushMessage.builder().flag((byte) 2).build();
            log.debug("client send heart beat message to server: {}", heartbeatPushMessage);
            context.writeAndFlush(heartbeatPushMessage);
        }
